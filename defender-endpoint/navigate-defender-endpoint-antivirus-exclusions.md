...
|[Defender for Endpoint Domain, URL and IP address Indicators](#defender-for-endpoint-indicators) |Allows a domain, URL or external IP address to be accessed that otherwise would be blocked by Defender.|A URL, domain or external IP address is identified as malicious even though it's not.  |
