...
| Compatibility with applications. For example, applications are crashing or experiencing unexpected behaviors when a device is onboarded to Microsoft Defender for Endpoint.  |  1. [Determine which component](#determining-the-component-that-is-causing-the-unexpected-behavior) is causing the issue <br/>2. If the source is Microsoft Defender Antivirus, then follow the steps in the **Performance Issues** section. |

## Determining the component that is causing the unexpected behavior
