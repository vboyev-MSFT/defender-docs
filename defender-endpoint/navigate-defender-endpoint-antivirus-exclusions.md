...
Compatablity with applications **Example** Applications are crashing or experiencing unexpected behaviors when a device is on-boarded to Microsoft Defender for Endpoint.  |  1.  [Determine the component](determining-the-component-that-is-causing-the-unexpected-behavior) which is causing the issue  2. If the source is Defender Anti-virus, then follow the steps in the **Performance Issues** section
