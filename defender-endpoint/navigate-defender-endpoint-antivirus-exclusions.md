...
| Compatibility with applications. For example, applications are crashing or experiencing unexpected behaviors when a device is onboarded to Microsoft Defender for Endpoint.  |  1. Identify which component is causing unexpected behavior (described in the next [section](#identify-which-component-is-causing-unexpected-behavior)). <br/>2. If the source is Microsoft Defender Antivirus, then follow the steps in the Performance issues section. |
