...
|[Defender for Endpoint File and Certificate Allow Indicators](#defender-for-endpoint-indicators)| Microsoft Defender Antivirus, Attack Surface Reduction Rules, Controlled Folder Access |A file or process signed by a certificate is identified as malicious even through it's not.|
|[Defender for Endpoint Domain/URL and IP address Indicators](#defender-for-endpoint-indicators) |SmartScreen, WebC ontent Filtering, Network Protection |SmartScreen false positive (FP) or to override a Web Content Filtering (WFC) block on a specific site.|
