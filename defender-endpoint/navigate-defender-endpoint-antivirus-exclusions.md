...
|[Defender for Endpoint Domain/URL and IP address Indicators](#defender-for-endpoint-indicators) |Allows a domain, URL or external IP address to be accessed that otherwise would be blocked by Defender.|SmartScreen false positive (FP) or to override a Web Content Filtering (WFC) block on a specific site.|
